...
var MapControls = function MapControls(object, domElement, options) {
  ...
  (init = function init(_scope) {
    ...
    _lodash2.default.each([-1, 1], function (orientation) {
  ...
  function calculateMinZoom(cam_pos, map_intersect) {
    return map_intersect.clone().add(cam_pos.clone().sub(map_intersect).normalize().multiplyScalar(scope.maxDistance));
  }
